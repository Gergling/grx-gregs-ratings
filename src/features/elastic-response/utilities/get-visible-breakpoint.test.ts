...
describe('getVisibleBreakpoint', () => {
  // Assumes ascending order as per the function's optimization note.
  ...
  describe('with a viewport smaller than the smallest breakpoint', () => {
    it('should return the smallest breakpoint', () => {
      expect(getVisibleBreakpoint(599, breakpoints)).toBe(600);
      expect(getVisibleBreakpoint(0, breakpoints)).toBe(600);
    });
  });
  ...
  describe('with a viewport size that falls between two breakpoints', () => {
    it('should return the next highest breakpoint (the ceiling)', () => {
      expect(getVisibleBreakpoint(601, breakpoints)).toBe(900);
      expect(getVisibleBreakpoint(899, breakpoints)).toBe(900);
      expect(getVisibleBreakpoint(901, breakpoints)).toBe(1200);
      expect(getVisibleBreakpoint(1199, breakpoints)).toBe(1200);
    });
  });
  ...
  describe('with a different set of breakpoints', () => {
    const mobileBreakpoints = [320, 480];
    it('should work correctly', () => {
      expect(getVisibleBreakpoint(300, mobileBreakpoints)).toBe(320);
      expect(getVisibleBreakpoint(400, mobileBreakpoints)).toBe(480);
